...
# Force @() so single-element arrays survive PS5.1 scriptblock unrolling;
# handle both raw-array and OData-wrapped {"value":[...]} responses
$saDiag = @(if ($null -ne $saDiagRaw.value) { $saDiagRaw.value } else { $saDiagRaw })
...
$kvDiag = @(if ($null -ne $kvDiagRaw.value) { $kvDiagRaw.value } else { $kvDiagRaw })
