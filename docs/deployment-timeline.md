...
**Note on Activity Log depth:** Azure Activity Log retains 90 days. The timestamps below represent the complete available record — nothing was omitted. The earliest event in the log (`2026-04-30T05:36:00Z`) is the first event Azure retained.
...
| `2026-04-30 11:47:19 -0400` | `b25b8e9b` | Live deploy complete — 78/78 push verified, STATUS.md, EOD captures committed |

**Elapsed from repo creation to live system: ~31 hours**
(2026-04-29T00:00 repo created → 2026-04-30T06:53Z first blob pushed)
...
| `2026-04-30T05:36:22Z` | `awacs-deploy-003` — deployment completes |
...
| `2026-04-30T05:56:37Z` | `deploy-01-sa` (Storage Account) + `deploy-02-kv` (Key Vault) start in parallel | |
| `2026-04-30T05:56:40Z` | **SAS token first written to Key Vault** | `current-write-sas` secret created |
| `2026-04-30T05:56:40Z` | Key Vault diagnostic settings written | KV → Log Analytics |
...
| `2026-04-30T05:57:05Z` | Blob diagnostic settings written | Storage → Log Analytics |
...
**Total IaC deployment time: ~61 seconds** (05:56:12Z → 05:57:15Z, resource creation to parent complete)
...
| UTC Timestamp | Event | Resource |
|---|---|---|
| `2026-04-30T05:57:57Z` | `Storage Blob Data Contributor` role assignment started | Storage account level |
| `2026-04-30T05:58:00Z` | `Storage Blob Data Contributor` role assignment complete | Required for user-delegation SAS generation |
| `2026-04-30T05:59:03Z` | `Key Vault Secrets Officer` role assignment started | KV level |
...
| `2026-04-30T06:53:08Z` → `06:53:15Z` | Initial batch of files pushed in first scheduled task run |

**Time from IaC complete to first successful push: approximately 56 minutes**
(05:57:15Z IaC done → 06:47:01Z RBAC finalized → 06:53:08Z first push)
...
With the V2 RBAC automation in place, this gap would shrink to ~5 minutes (task scheduler registration + first fire).
...
**Total outage duration: 6 days 17 hours 57 minutes** (2026-05-01T06:41Z → 2026-05-08T00:38Z)
...
## Summary: What This Timeline Proves

| Claim | Evidence |
|---|---|
| "Deploy from zero to running in one script" | IaC phase: 61 seconds from first resource to deployment complete |
| "WORM immutability is built-in" | Immutability policy set at `2026-04-30T05:57:09Z` — before first push |
| "All RBAC is code-defined" | Role assignments visible in Activity Log with deployment names |
| "Logging is on from day one" | Diagnostic settings written at `2026-04-30T05:57:05Z` — same deployment run |
| "Workstation push is automated" | First push `2026-04-30T06:53:08Z` with no manual blob operation |
| "Teardown works" | Two clean teardown cycles visible in Activity Log before final deploy |
| "System is live and accumulating data" | 97 ledger entries as of 2026-05-08T00:38Z |
...
- **Subscription:** `49521d08-4a34-4355-a069-919af69ad956`
- **Pulled:** 2026-05-08 by Claude Code session
